...
> **สถานะ:** ตรวจสอบเสร็จ ยังไม่ได้แก้ไขโค้ด — เอกสารนี้เป็นรายงาน audit เท่านั้น
...
### 1. RotaryEncoder — ISR callback signature ผิด → crash ทันทีที่หมุน
...
### 2. ServoTester — คำนวณ duty cycle ผิดทั้งไฟล์ → เครื่องมือใช้งานไม่ได้เลย
...
### 3. RC522 — อ่าน UID ผิดสำหรับการ์ด 7-byte UID (พบได้บ่อยมาก)
...
### 4. MAX31855 — enum ชนกับ macro ชื่อเดียวกัน → fault status ผิดทุกครั้ง
...
### 5. MAX6675 — อ่านอุณหภูมิผิดที่ ≥512°C
...
### 6. MAX7219 — negative x ทำให้ scroll text พังทุกครั้ง
...
### 7-8. NeoPixel — Scanner effect แฮงค์เครื่อง / หาร 0
...
### 9. SoftUART — `Init()` ไม่กัน baud=0 ทั้งที่เอกสารอ้างว่ากันแล้ว
...
### 10. SoftUART — RX ring buffer ไม่เคยถูกเติมข้อมูล (ยืนยันซ้ำ ยังไม่แก้)
**ไฟล์:** `SoftUART.c:72-111` — `SoftUART_Available()` คืน `rx_count` ที่ไม่เคยถูกเพิ่มค่าที่ไหนเลย
คืน 0 เสมอ ตรงตามที่ LIMITATIONS.md บันทึกไว้ (ยังไม่ถูกแก้)

### 11. WaterFlow_YFS201 — นับพัลส์ผิดถ้าใช้หลายตัวพร้อมกัน
...
### 12. TM1637 — `DisplayNumber` พังที่ INT16_MIN (ขัดแย้งกับ LIMITATIONS.md)
...
### 13. TM1650 — บั๊กเดียวกับ TM1637 ที่ INT16_MIN (ไม่เคยบันทึกไว้)
...
### 14. CircularBuffer — ไม่ thread-safe จริงตามที่เอกสารอ้าง
...
### 15. AT24Cxx — bounds check โดน bypass ด้วย integer overflow
...
### 16. GPS_NEO6M — `fix_valid` ไม่เคยถูกล้างเมื่อ GPS หลุด lock
...
### 17. SoundSensor_KY038 — สมมติ ADC 12-bit ผิด (ชิปนี้ 10-bit) → ตรวจจับเสียงปรบมือไม่ได้เลย
...
### 18. SoilMoisture_YL69 — บั๊กเดียวกัน (สมมติ ADC 12-bit)
...
### 19. OLED — ไม่มี `initialized` guard เลยทั้งไฟล์ + bug ใน FillTriangle
...
### 20. PID — ไม่กัน `dt=0` → output เป็น NaN/Inf ตลอดไป
...
### 21. P10 — ความกว้างที่ไม่ใช่เลขคูณ 8 ทำให้ pixel เพี้ยนข้ามแถว
...
### 22. LCD1602_I2C — ไม่มี NULL/initialized guard เลย กระทบ LCDMenu ด้วย
...
### 23. NTC10K — `InitWithConfig` ไม่เช็ค NULL param
**ไฟล์:** `NTC10K.c:137-146` — ต่างจาก `NTC_Init()` ปกติที่ปลอดภัย ฟังก์ชันนี้ crash ทันทีถ้า
ส่ง `config` เป็น NULL

### 24. IR — ข้อมูลที่ ISR แชร์ไม่มี `volatile` และไม่มี critical section
...
### 25. HX711 — `initialized=1` ถูกตั้งก่อนเช็คว่า init สำเร็จจริง
...
### 26. Buzzer — `FrequencySweep` หาร 0 โดยไม่ตั้งใจ (ไม่กระทบผลลัพธ์จริง)
...
### 27. HC05 — มี field `rx_buf`/`rx_head`/`rx_tail` ที่ไม่เคยถูกใช้จริง
**ไฟล์:** `HC05.c` — dead/vestigial state ไม่ใช่บั๊กเชิงพฤติกรรม (I/O จริงผ่าน
`USART_Available`/`USART_Read` ตรงๆ) แต่ควร cleanup

### 28. WS2812Matrix/WS2815Matrix — UTF-8 truncated sequence อ่านเกิน buffer
...
### 29. WS2815Matrix — gradient pattern หาร 0 ถ้า width/height=1
